...
let render engine img ~pos ?src ?size transform =
  let src = src |? Box2.v V2.zero img.Tartine_image.size |> box2_to_sdl in
  let size = size |? img.Tartine_image.size in
  let elt = transform {
    dst = Box2.v pos size;
  ...
  if Float.abs elt.angle < 0.01 && flip = Sdl.Flip.none then
    Sdl.render_copy ~src ~dst engine.renderer
      img.Tartine_image.texture
  else
    Sdl.render_copy_ex
      ~src ~dst
      engine.renderer img.Tartine_image.texture
